...
import loading_gif from "../images/Edit_Ing/loading_spinner_3.gif"
import tip from "../images/Edit_Ing/팁.png"
...
const Edit = () => {
  ...
  const [qnaList, setQnaList] = useState([new Qna("", "")])
  ...
  const editButtonClick = () => {
    ...
    axios.post(server_url + "/api/edit" + `?username=${localStorage.getItem(
        'username')}`,
        JSON.stringify(data),
  ...
  const saveButtonClick = () => {
    const data = {"title": coverLetterTitle, "qna": qnaList}
    axios.post(
        server_url + "/api/postcoverletter" + `?username=${localStorage.getItem(
            'username')}&state=-1`,
        JSON.stringify(data),
        {headers: {"Content-Type": "application/json"}})
    .then(response => {
  ...
  useEffect(() => {
    if (location.state) {
      const {title, qnas} = location.state;
    ...
    return (
        <div className={"edit_container"}>
          ...
          {loading ? <div className={"loading_overlay"}>
                ...
                <img className={"loading_gif"} src={loading_gif}/>
              </div> :
              <div/>}
          <div className={"contents_container"}>
            ...
            <div className={"button_div"}>
              ...
              <button onClick={saveButtonClick} className={"API_edit"}>저장하기
              </button>
            </div>
          </div>
        </div>
